(() => {
  ...
  const SOUNDS = {
    pulse: { label: 'Pulse', file: 'alarm-pulse.wav', description: 'Klarer Doppelimpuls für Event-Alarme', symbol: 'P' },
    siren: { label: 'Siren', file: 'alarm-siren.wav', description: 'Durchdringender Alarmton für Bubble-Alarme', symbol: 'S' },
    chime: { label: 'Chime', file: 'alarm-chime.wav', description: 'Dreistufiger Klang für RSS-Alarme', symbol: 'C' },
  };
  const TYPE_LABEL = { bubble: 'Bubble', gw: 'GW Bubble', custom: 'Event · Individual / Hellcat', individual: 'Individual · Investment / Building / Training', rss: 'RSS · Tiles / Trucks / Schmuggler' };
  ...
  function renderAlarmModal(id, templateKey) {
    ...
    const template = templateKey === 'gw' ? { type: 'gw', title: 'GW-Zeitfenster', warnings: [60, 30, 15], repeat: 'once', sound: 'siren', protected: true } : templateKey === 'individual' ? { type: 'individual', title: 'Individual-Alarm', warnings: [15], repeat: 'once', sound: 'pulse', protected: false } : templateKey === 'rss' ? { type: 'rss', title: 'RSS-Alarm', warnings: [15], repeat: 'once', sound: 'chime', protected: false } : templateKey === 'custom' ? { type: 'custom', title: 'Mein TGM-Event', warnings: [15], repeat: 'once', sound: 'pulse', protected: false } : { type: 'bubble', title: 'Bubble-Zeitfenster', warnings: [60, 15], repeat: 'once', sound: 'siren', protected: true };
  ...
  function renderAlarmList(limit = Infinity) {
    ...
    if (!alarms.length) return `<div class="card empty"><div class="empty-icon">+</div><h3>Noch kein Gaming-Alarm</h3><p>Lege eine Bubble, ein GW-Schutzfenster, ein Event, einen Individual- oder einen RSS-Alarm an.</p><div class="actions"><button class="btn primary" type="button" data-action="new-alarm" data-template="bubble">Bubble anlegen</button></div></div>`;
  ...
  function renderSoundsView() {
    return `<section><div class="section-head"><div><div class="eyebrow">AUDIO</div><h2>Gaming-Alarmtöne</h2></div><button class="btn primary" type="button" data-action="unlock-audio">${state.preferences.audioEnabled ? 'Audio aktiviert' : 'Audio aktivieren'}</button></div><div class="note"><strong>Einmal aktivieren, dauerhaft bereit.</strong><br>Browser erlauben Audio erst nach einer Nutzeraktion. Danach werden die lokalen Töne für Vorwarnungen, Hauptereignisse und GW-Bubble-Enden verwendet.</div><div class="grid grid-3" style="margin-top:16px">${Object.entries(SOUNDS).map(([key, sound]) => `<article class="card pad"><div class="stat-icon">${esc(sound.symbol)}</div><h2>${esc(sound.label)}</h2><p class="muted">${esc(sound.description)}</p><div class="badges"><span class="badge ${key === 'siren' ? 'red' : key === 'chime' ? 'blue' : 'gold'}">${key === 'siren' ? 'BUBBLE' : key === 'chime' ? 'RSS' : 'EVENT'}</span></div><button class="btn secondary full" type="button" data-action="preview-sound" data-sound="${esc(key)}">${state.preferences.audioEnabled ? 'Ton anhören' : 'Audio zuerst aktivieren'}</button></article>`).join('')}</div></section>`;
  }

  function renderPlansView() {
    const currentTier = effectiveTierKey();
    const active = activeAccount();
    const familyAccess = familyAccessForAccount(active);
    const trialPanel = familyAccess ? `<div class="note trial-panel active"><strong>Godfather Lifetime · kostenfrei</strong><br>Für ${esc(active.name)} dauerhaft freigeschaltet.</div>` : freeTrialActive() ? `<div class="note trial-panel active"><strong>Kostenlose Testphase läuft</strong><br>Alle Funktionen sind freigeschaltet. ${esc(trialCountdownLabel())}</div>` : freeTrialAvailable() ? `<div class="note trial-panel"><strong>72 Stunden kostenlos testen</strong><br>Nutze den vollständigen Funktionsumfang drei Tage lang auf diesem Gerät.<div class="actions"><button class="btn primary" type="button" data-action="start-free-trial">Testphase starten</button></div></div>` : `<div class="note trial-panel expired"><strong>Testphase bereits genutzt</strong><br>Die kostenlose Testphase kann auf diesem Gerät nur einmal gestartet werden.</div>`;
    return `<section><div class="section-head"><div><div class="eyebrow">PREISE UND FUNKTIONSUMFANG</div><h2>Pläne und Preise</h2><p>Vergleiche den verfügbaren Umfang für Accounts und Alarme.</p></div><span class="badge gold">${familyAccess ? 'GODFATHER LIFETIME · KOSTENFREI' : freeTrialActive() ? 'TESTPHASE · ' : 'AKTIV · '}${esc(TIER_PRICING[currentTier].name)}</span></div><div class="note plan-intro"><strong>Auswahl auf diesem Gerät</strong><br>${familyAccess ? `Godfather Lifetime ist für ${esc(active.name)} dauerhaft kostenfrei freigeschaltet.` : 'Plan, Limits und Teststatus werden auf diesem Gerät gespeichert.'}</div>${trialPanel}<div class="plan-grid">${TIER_ORDER.map((tier) => { const plan = TIER_PRICING[tier]; const current = tier === currentTier; const familyPlan = familyAccess && tier === FAMILY_ACCESS_TIER; const familyLocked = familyAccess && tier !== FAMILY_ACCESS_TIER; return `<article class="card plan ${current ? 'current' : ''}"><div class="plan-badge">${familyPlan ? 'KOSTENFREI' : current ? 'AKTIV' : tier === 'free' ? 'BASIS' : tier === 'godfather' ? 'UNBEGRENZT' : 'ERWEITERT'}</div><h3>${esc(plan.name)}</h3><div class="plan-price">${familyPlan ? 'Kostenfrei' : formatPlanPrice(plan.eur.monthly, 'EUR')} <span>${familyPlan ? '/ Lifetime' : '/ Monat'}</span></div>${familyPlan ? `<div class="plan-note">Dauerhaft kostenfrei für diesen Account</div>` : plan.annualSavingPercent ? `<div class="plan-note">Bei jährlicher Zahlung ${plan.annualSavingPercent}% weniger als bei 12 Monatszahlungen</div>` : '<div class="plan-note">Grundumfang für die lokale Alarmplanung</div>'}<div class="plan-limit-grid"><div class="plan-limit"><span>Accounts</span><strong>${formatPlanLimit(plan.limits.accounts)}</strong></div><div class="plan-limit"><span>Bubble-Alarme je Account</span><strong>${formatPlanLimit(plan.limits.perAccount.bubbleAlarms)}</strong></div><div class="plan-limit"><span>Event-Alarme je Account</span><strong>${formatPlanLimit(plan.limits.perAccount.eventAlarms)}</strong></div><div class="plan-limit"><span>Individual-Alarme je Account</span><strong>${formatPlanLimit(plan.limits.perAccount.individualAlarms)}</strong></div><div class="plan-limit"><span>RSS-Alarme je Account</span><strong>${formatPlanLimit(plan.limits.perAccount.rssAlarms)}</strong></div></div><ul>${TIER_FEATURES[tier].map((feature) => `<li>${esc(feature)}</li>`).join('')}</ul><div class="plan-price-list"><div class="plan-price-row plan-price-head"><span>Laufzeit</span><span>EUR</span><span>USD (Store)</span></div>${BILLING_PERIODS.map((period) => `<div class="plan-price-row ${period === 'monthly' ? 'featured' : ''}"><span>${BILLING_LABELS[period]}</span><strong>${familyPlan && period === 'lifetime' ? 'Kostenfrei' : formatPlanPrice(plan.eur[period], 'EUR')}</strong><strong>${familyPlan && period === 'lifetime' ? 'Kostenfrei' : formatPlanPrice(plan.usdStore[period], 'USD')}</strong></div>`).join('')}</div><button class="btn ${current || familyLocked ? 'ghost' : 'secondary'} full" type="button" data-action="select-tier" data-tier="${tier}" ${current || familyLocked ? 'disabled' : ''}>${familyPlan ? 'Dauerhaft freigeschaltet' : familyLocked ? 'Nicht erforderlich' : current ? 'Aktiver Plan' : 'Auswählen'}</button></article>`; }).join('')}</div></section>`;
  }
  ...
  function saveAlarm(id) {
    ...
    if (type === 'individual' && individualAlarms >= individualLimit) return showToast(`${plan.name} erlaubt je Account ${formatPlanLimit(plan.limits.perAccount.individualAlarms)} Individual-Alarm${plan.limits.perAccount.individualAlarms === 1 ? '' : 'e'}.`);
    if (type === 'rss' && rssAlarms >= rssLimit) return showToast(`${plan.name} erlaubt je Account ${formatPlanLimit(plan.limits.perAccount.rssAlarms)} RSS-Alarm${plan.limits.perAccount.rssAlarms === 1 ? '' : 'e'}.`);
    const record = {
