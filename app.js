(() => {
  ...
  const TIER_ORDER = ['free','streetBoss','caporegime','underboss','godfather'];
  ...
  const FOUNDER_ACCESS_TIER = 'godfather';
  const FOUNDER_ACCOUNT_NAMES = Object.freeze(['TGMack', 'TGMkellz', 'TGMj9', 'TGMvany', 'TGMred']);
  const FOUNDER_ACCOUNT_NAME_KEYS = new Set(FOUNDER_ACCOUNT_NAMES.map((name) => name.toLowerCase()));
  const TIER_PRICING = {free:{name:'Free',limits:{accounts:1,alarms:2,events:1,perAccount:{bubbleAlarms:1,eventAlarms:1}},eur:{weekly:0,monthly:0,sixMonth:0,yearly:0,lifetime:0},usdDirect:{weekly:0,monthly:0,sixMonth:0,yearly:0,lifetime:0},usdStore:{weekly:0,monthly:0,sixMonth:0,yearly:0,lifetime:0},jpyDirect:{weekly:0,monthly:0,sixMonth:0,yearly:0,lifetime:0},jpyStore:{weekly:0,monthly:0,sixMonth:0,yearly:0,lifetime:0},annualSavingPercent:null},streetBoss:{name:'Street Boss',limits:{accounts:2,alarms:4,events:2,perAccount:{bubbleAlarms:1,eventAlarms:1}},eur:{weekly:4.99,monthly:14.99,sixMonth:79.99,yearly:129.99,lifetime:199.99},usdDirect:{weekly:5.79,monthly:17.37,sixMonth:92.77,yearly:150.77,lifetime:231.91},usdStore:{weekly:5.99,monthly:16.99,sixMonth:89.99,yearly:149.99,lifetime:214.99},jpyDirect:{weekly:924,monthly:2777,sixMonth:14815,yearly:24072,lifetime:37042},jpyStore:{weekly:1000,monthly:2800,sixMonth:14800,yearly:24000,lifetime:37000},annualSavingPercent:17},caporegime:{name:'Caporegime',limits:{accounts:3,alarms:6,events:3,perAccount:{bubbleAlarms:1,eventAlarms:1}},eur:{weekly:7.99,monthly:24.99,sixMonth:129.99,yearly:199.99,lifetime:299.99},usdDirect:{weekly:9.26,monthly:28.99,sixMonth:150.72,yearly:231.83,lifetime:347.73},usdStore:{weekly:9.99,monthly:27.99,sixMonth:149.99,yearly:229.99,lifetime:319.99},jpyDirect:{weekly:1479,monthly:4629,sixMonth:24077,yearly:37038,lifetime:55557},jpyStore:{weekly:1500,monthly:4600,sixMonth:24000,yearly:37000,lifetime:55000},annualSavingPercent:23},underboss:{name:'Underboss',limits:{accounts:5,alarms:10,events:5,perAccount:{bubbleAlarms:1,eventAlarms:1}},eur:{weekly:9.99,monthly:34.99,sixMonth:179.99,yearly:299.99,lifetime:449.99},usdDirect:{weekly:11.58,monthly:40.56,sixMonth:208.77,yearly:347.79,lifetime:521.87},usdStore:{weekly:11.99,monthly:39.99,sixMonth:199.99,yearly:349.99,lifetime:479.99},jpyDirect:{weekly:1850,monthly:6483,sixMonth:33332,yearly:55552,lifetime:83358},jpyStore:{weekly:1900,monthly:6500,sixMonth:33000,yearly:56000,lifetime:83000},annualSavingPercent:17},godfather:{name:'Godfather',limits:{accounts:null,alarms:null,events:null,perAccount:{bubbleAlarms:null,eventAlarms:null}},eur:{weekly:19.99,monthly:69.99,sixMonth:399.99,yearly:599.99,lifetime:799.99},usdDirect:{weekly:23.18,monthly:81.1,sixMonth:463.96,yearly:695.68,lifetime:927.64},usdStore:{weekly:22.99,monthly:79.99,sixMonth:449.99,yearly:699.99,lifetime:899.99},jpyDirect:{weekly:3702,monthly:12956,sixMonth:74086,yearly:111061,lifetime:148106},jpyStore:{weekly:3700,monthly:13000,sixMonth:74000,yearly:111000,lifetime:148000},annualSavingPercent:17}};
  const TIER_FEATURES = {free:['1 Account','1 Bubble-Alarm pro Account','1 Event-Alarm pro Account','Alle drei Gaming-Töne'],streetBoss:['2 Accounts','Je 1 Bubble-Alarm pro Account','Je 1 Event-Alarm pro Account','GW-Schutzfenster mit Vorwarnungen'],caporegime:['3 Accounts','Je 1 Bubble-Alarm pro Account','Je 1 Event-Alarm pro Account','GW-Zyklen und vollständige Backup-Funktionen'],underboss:['5 Accounts','Je 1 Bubble-Alarm pro Account','Je 1 Event-Alarm pro Account','Multi-Account-Profi-Stufe mit mehr Alarm-/Event-Spielraum'],godfather:['Unbegrenzte Accounts','Je 1 Bubble-Alarm pro Account','Je 1 Event-Alarm pro Account','Die vollständige lokale Alarmzentrale']};
  const DEFAULT_PREFS = {
  ...
  const isFounderAccountName = (accountName) => typeof accountName === 'string' && FOUNDER_ACCOUNT_NAME_KEYS.has(accountName.trim().toLowerCase());
  const founderAccessForAccount = (account = activeAccount()) => Boolean(account && isFounderAccountName(account.name));
  const effectiveTierKey = () => founderAccessForAccount() ? FOUNDER_ACCESS_TIER : (freeTrialActive() ? FREE_TRIAL_TIER : (state.tier || 'free'));
  ...
  function renderPlansView() {
    ...
    const founderAccess = founderAccessForAccount(active);
    const trialPanel = founderAccess ? `<div class="note trial-panel active"><strong>Godfather Lifetime · kostenfrei</strong><br>Für ${esc(active.name)} dauerhaft freigeschaltet.</div>` : freeTrialActive() ? `<div class="note trial-panel active"><strong>Kostenlose Testphase läuft</strong><br>Alle Funktionen sind freigeschaltet. ${esc(trialCountdownLabel())}</div>` : freeTrialAvailable() ? `<div class="note trial-panel"><strong>72 Stunden kostenlos testen</strong><br>Nutze den vollständigen Funktionsumfang drei Tage lang auf diesem Gerät.<div class="actions"><button class="btn primary" type="button" data-action="start-free-trial">Testphase starten</button></div></div>` : `<div class="note trial-panel expired"><strong>Testphase bereits genutzt</strong><br>Die kostenlose Testphase kann auf diesem Gerät nur einmal gestartet werden.</div>`;
    return `<section><div class="section-head"><div><div class="eyebrow">PREISE UND FUNKTIONSUMFANG</div><h2>Pläne und Preise</h2><p>Vergleiche den verfügbaren Umfang für Accounts und Alarme.</p></div><span class="badge gold">${founderAccess ? 'GODFATHER LIFETIME · KOSTENFREI' : freeTrialActive() ? 'TESTPHASE · ' : 'AKTIV · '}${esc(TIER_PRICING[currentTier].name)}</span></div><div class="note plan-intro"><strong>Auswahl auf diesem Gerät</strong><br>${founderAccess ? `Godfather Lifetime ist für ${esc(active.name)} dauerhaft kostenfrei freigeschaltet.` : 'Plan, Limits und Teststatus werden auf diesem Gerät gespeichert.'}</div>${trialPanel}<div class="plan-grid">${TIER_ORDER.map((tier) => { const plan = TIER_PRICING[tier]; const current = tier === currentTier; const founderPlan = founderAccess && tier === FOUNDER_ACCESS_TIER; const founderLocked = founderAccess && tier !== FOUNDER_ACCESS_TIER; return `<article class="card plan ${current ? 'current' : ''}"><div class="plan-badge">${founderPlan ? 'KOSTENFREI' : current ? 'AKTIV' : tier === 'free' ? 'BASIS' : tier === 'godfather' ? 'UNBEGRENZT' : 'ERWEITERT'}</div><h3>${esc(plan.name)}</h3><div class="plan-price">${founderPlan ? 'Kostenfrei' : formatPlanPrice(plan.eur.monthly, 'EUR')} <span>${founderPlan ? '/ Lifetime' : '/ Monat'}</span></div>${founderPlan ? `<div class="plan-note">Dauerhaft kostenfrei für diesen Account</div>` : plan.annualSavingPercent ? `<div class="plan-note">Bei jährlicher Zahlung ${plan.annualSavingPercent}% weniger als bei 12 Monatszahlungen</div>` : '<div class="plan-note">Grundumfang für die lokale Alarmplanung</div>'}<div class="plan-limit-grid"><div class="plan-limit"><span>Accounts</span><strong>${formatPlanLimit(plan.limits.accounts)}</strong></div><div class="plan-limit"><span>Bubble-Alarme je Account</span><strong>${formatPlanLimit(plan.limits.perAccount.bubbleAlarms)}</strong></div><div class="plan-limit"><span>Event-Alarme je Account</span><strong>${formatPlanLimit(plan.limits.perAccount.eventAlarms)}</strong></div></div><ul>${TIER_FEATURES[tier].map((feature) => `<li>${esc(feature)}</li>`).join('')}</ul><div class="plan-price-list"><div class="plan-price-row plan-price-head"><span>Laufzeit</span><span>EUR</span><span>USD (Store)</span></div>${BILLING_PERIODS.map((period) => `<div class="plan-price-row ${period === 'monthly' ? 'featured' : ''}"><span>${BILLING_LABELS[period]}</span><strong>${founderPlan && period === 'lifetime' ? 'Kostenfrei' : formatPlanPrice(plan.eur[period], 'EUR')}</strong><strong>${founderPlan && period === 'lifetime' ? 'Kostenfrei' : formatPlanPrice(plan.usdStore[period], 'USD')}</strong></div>`).join('')}</div><button class="btn ${current || founderLocked ? 'ghost' : 'secondary'} full" type="button" data-action="select-tier" data-tier="${tier}" ${current || founderLocked ? 'disabled' : ''}>${founderPlan ? 'Dauerhaft freigeschaltet' : founderLocked ? 'Nicht erforderlich' : current ? 'Aktiver Plan' : 'Auswählen'}</button></article>`; }).join('')}</div></section>`;
  }
