(() => {
  ...
  function renderBottomNav() {
    return `<nav class="mobile-bottom-nav" aria-label="App-Navigation"><button type="button" class="${view === 'today' ? 'active' : ''}" data-action="view" data-view="today"><span class="bottom-icon">⌂</span><span>DASHBOARD</span></button><button type="button" class="${view === 'alarms' ? 'active' : ''}" data-action="view" data-view="alarms"><span class="bottom-icon">♧</span><span>ALARME</span>${state.alarms.filter((alarm) => alarm.active).length ? `<b>${state.alarms.filter((alarm) => alarm.active).length}</b>` : ''}</button><button type="button" class="${view === 'today' ? 'active-soft' : ''}" data-action="view" data-view="today"><span class="bottom-icon">⌖</span><span>KARTE</span></button><button type="button" class="${view === 'sounds' ? 'active' : ''}" data-action="view" data-view="sounds"><span class="bottom-icon">◉</span><span>FUNK</span></button><button type="button" class="${view === 'settings' ? 'active' : ''}" data-action="view" data-view="settings"><span class="bottom-icon">▦</span><span>SYSTEM</span></button></nav>`;
  }

  function render() {
    const previousNav = app.querySelector('.nav');
    if (previousNav) navScrollLeft = previousNav.scrollLeft;
    app.innerHTML = `<div class="app-shell"><header class="topbar"><div class="toprow"><button class="crest-button" type="button" data-action="view" data-view="today" aria-label="Dashboard"><span class="brand-mark" aria-hidden="true">TGM</span></button><div class="header-title">TGM ALARM-CENTER</div><button class="live-button" type="button" data-action="view" data-view="alarms"><span class="live-dot"></span><span>LIVE</span></button></div>${activeAccount() ? `<button class="account-strip" type="button" data-action="account-menu" aria-label="Account wechseln"><span class="account-dot"></span>${esc(activeAccount().name)}<span class="account-chevron">⌄</span></button>` : ''}<nav class="nav" aria-label="Hauptnavigation">${navButton('today', 'Übersicht')}${navButton('alarms', 'Alarme')}${navButton('accounts', 'Accounts')}${navButton('sounds', 'Gaming-Töne')}${navButton('plans', 'Pläne & Preise')}${navButton('settings', 'Einstellungen')}</nav></header><main class="main">${view === 'today' ? renderDashboard() : view === 'alarms' ? renderAlarmsView() : view === 'accounts' ? renderAccountsView() : view === 'sounds' ? renderSoundsView() : view === 'plans' ? renderPlansView() : renderSettingsView()}</main><footer class="footer">TGM ALARM CENTER · lokale Speicherung · lokale Gaming-Alarmtöne · ${SCHEMA_VERSION.toString()}</footer>${renderBottomNav()}</div>`;
