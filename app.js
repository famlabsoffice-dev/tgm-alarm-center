(() => {
  ...
  function renderDashboard() {
    ...
    const alarms = state.alarms.filter((alarm) => alarm.active).slice(0, 3);
    const nextAlarm = next ? nextMomentAlarm(next) : alarms[0];
    const nextAt = next?.at || nextAlarm?.eventAt || null;
    return `<section class="reference-dashboard"><div class="reference-heading"><h1>Dein nächster Alarm.</h1><strong>Immer im Blick.</strong></div><div class="reference-brand"><img src="assets/tgm-alarm-center-icon.png" alt="TGM Alarm Center" loading="eager"><div><b>TGM</b><span>ALARM CENTER</span></div></div><section class="reference-next"><div class="reference-section-title"><span class="reference-symbol">▣</span><span>ALS NÄCHSTES</span></div><div class="reference-next-card"><div class="reference-alarm-orb">♧</div><div class="reference-next-copy"><div class="reference-next-line"><div><h2>${esc(nextAlarm?.title || 'Bubble Alarm')}</h2><p>${esc(nextAlarm ? typeLabel(nextAlarm.type) : 'Täglicher Alarm')}</p></div><span class="reference-active">AKTIV <i></i></span></div><small>LÖST AUS IN</small><strong class="reference-countdown">${nextAt ? countdown(nextAt) : '00 : 32 : 45'}</strong><div class="reference-time-labels"><span>STD</span><span>MIN</span><span>SEK</span></div><button class="reference-open" type="button" data-action="view" data-view="alarms"><span>♧</span>Alarm anzeigen <b>›</b></button></div></div></section><div class="reference-list-head"><h2>DEINE ALARME</h2><button type="button" data-action="new-alarm" data-template="custom" aria-label="Alarm hinzufügen">＋</button></div><div class="reference-alarm-list">${alarms.length ? alarms.map((alarm) => `<article class="reference-alarm-row"><span class="reference-row-icon">${alarm.type === 'gw' ? 'ϟ' : alarm.type === 'individual' ? '◒' : '⌁'}</span><div><strong>${esc(alarm.title)}</strong><small>${esc(typeLabel(alarm.type))}</small><em>${formatTime(alarm.eventAt)}</em></div><button class="reference-switch ${alarm.active ? 'on' : ''}" type="button" data-action="toggle-alarm" data-id="${esc(alarm.id)}" aria-label="Alarm umschalten"><i></i></button><span class="reference-chevron">›</span></article>`).join('') : `<div class="reference-empty">Noch keine Alarme angelegt.</div>`}</div></section>`;
  }
