(() => {
  ...
  const TIER_PRICING = {
    ...
    caporegime: { name: 'Caporegime', limits: { accounts: 3, alarms: 6, events: 3, perAccount: { bubbleAlarms: 1, eventAlarms: 1 } }, eur: { weekly: 2.99, monthly: 8.99, sixMonth: 49.99, yearly: 79.99, lifetime: 149.99 }, usdDirect: { weekly: 3.47, monthly: 10.43, sixMonth: 57.99, yearly: 92.79, lifetime: 173.99 }, usdStore: { weekly: 2.99, monthly: 9.99, sixMonth: 54.99, yearly: 89.99, lifetime: 159.99 }, annualSavingPercent: 33 },
    godfather: { name: 'Godfather', limits: { accounts: Infinity, alarms: Infinity, events: Infinity, perAccount: { bubbleAlarms: Infinity, eventAlarms: Infinity } }, eur: { weekly: 4.99, monthly: 14.99, sixMonth: 89.99, yearly: 119.99, lifetime: 229.99 }, usdDirect: { weekly: 5.79, monthly: 17.39, sixMonth: 104.39, yearly: 139.19, lifetime: 266.79 }, usdStore: { weekly: 4.99, monthly: 16.99, sixMonth: 94.99, yearly: 129.99, lifetime: 239.99 }, annualSavingPercent: 33 },
  };
  const TIER_FEATURES = {
    free: ['1 Kommando', '1 Bubble-Alarm pro Kommando', '1 Event-Alarm pro Kommando', 'Alle drei Gaming-Töne'],
    streetBoss: ['2 Kommandos', 'Je 1 Bubble-Alarm pro Kommando', 'Je 1 Event-Alarm pro Kommando', 'GW-Schutzfenster mit Vorwarnungen'],
    caporegime: ['3 Kommandos', 'Je 1 Bubble-Alarm pro Kommando', 'Je 1 Event-Alarm pro Kommando', 'GW-Zyklen und vollständige Backup-Funktionen'],
    godfather: ['Unbegrenzte Kommandos', 'Je 1 Bubble-Alarm pro Kommando', 'Je 1 Event-Alarm pro Kommando', 'Die vollständige lokale Alarmzentrale'],
  };
  const DEFAULT_PREFS = {
  ...
  function saveAlarm(id) {
    ...
    if ((type === 'bubble' || type === 'gw') && bubbleAlarms >= plan.limits.perAccount.bubbleAlarms) return showToast(`${plan.name} erlaubt je Kommando ${formatPlanLimit(plan.limits.perAccount.bubbleAlarms)} Bubble-Alarm${plan.limits.perAccount.bubbleAlarms === 1 ? '' : 'e'}.`);
    if (type === 'custom' && eventAlarms >= plan.limits.perAccount.eventAlarms) return showToast(`${plan.name} erlaubt je Kommando ${formatPlanLimit(plan.limits.perAccount.eventAlarms)} Event-Alarm${plan.limits.perAccount.eventAlarms === 1 ? '' : 'e'}.`);
    const record = {
