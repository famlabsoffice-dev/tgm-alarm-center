(() => {
  ...
  const FOUNDER_ACCOUNT_NAMES = new Set(['TGMack', 'TGMkellz', 'TGMj9', 'TGMvany', 'TGMred'].map((name) => name.toLowerCase()));
  ...
  const activeFounderAccount = () => {
    const account = state?.accounts?.find((item) => item.id === state.activeAccountId);
    return Boolean(account?.name && FOUNDER_ACCOUNT_NAMES.has(account.name.trim().toLowerCase()));
  };
  const effectiveTierKey = () => activeFounderAccount() || freeTrialActive() ? FREE_TRIAL_TIER : 'free';
  ...
  function renderAlarmModal(id, templateKey) {
    ...
    const template = templateKey === 'gw'
      ? { type: 'gw', title: 'Massacre Alarm', warnings: [60, 30, 15], repeat: 'gw5d', sound: 'siren', protected: true }
      : templateKey === 'individual'
        ? { type: 'individual', title: 'Individual Timer', warnings: [15], repeat: 'once', sound: 'pulse', protected: false }
        : templateKey === 'rss'
          ? { type: 'rss', title: 'RSS Timer', warnings: [15], repeat: 'once', sound: 'chime', protected: false }
          : templateKey === 'custom'
            ? { type: 'custom', title: 'Event Alarm', warnings: [15], repeat: 'once', sound: 'pulse', protected: false }
            : { type: 'bubble', title: 'Bubble Alarm', warnings: [60], repeat: 'once', sound: 'siren', protected: true };
  ...
  function renderDashboard() {
    ...
    const accountAlarmsList = visibleAlarms();
    const gwAlarms = accountAlarmsList.filter((alarm) => alarm.type === 'gw' && alarm.repeat === 'gw5d' && alarm.active);
    const nextGw = gwAlarms.map((alarm) => ({ alarm, eventAt: nextOccurrence(alarm) })).filter((item) => item.eventAt).sort((a, b) => a.eventAt - b.eventAt)[0] || null;
    const upcoming = accountAlarmsList.filter((alarm) => alarm.active).flatMap((alarm) => {
      const first = nextOccurrence(alarm);
      if (!first) return [];
      const events = [{ alarm, at: first }];
      if (alarm.repeat === 'gw5d') {
        events.push({ alarm, at: first + FIVE_DAYS_MS });
        events.push({ alarm, at: first + 2 * FIVE_DAYS_MS });
      } else if (alarm.repeat === 'daily') {
        events.push({ alarm, at: first + DAY_MS });
        events.push({ alarm, at: first + 2 * DAY_MS });
      }
      return events;
    }).filter((item) => item.at > now()).sort((a, b) => a.at - b.at).filter((item, index, all) => all.findIndex((candidate) => candidate.alarm.id === item.alarm.id && candidate.at === item.at) === index).slice(0, 3);
    const cycleDays = Array.from({ length: 7 }, (_, index) => ({ day: index + 1, active: Boolean(nextGw) && index % 5 === 0 }));
    const nextGwCountdown = nextGw ? countdown(nextGw.eventAt) : '—';
    const nextGwDate = nextGw ? new Intl.DateTimeFormat('de-DE', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(nextGw.eventAt)) : 'Noch kein Massacre Alarm geplant';
  ...
  function handleChange(event) {
    ...
    if (event.target.id === 'eTypeVisible') {
      const target = document.getElementById('eType');
      if (target) { target.value = event.target.value; target.dispatchEvent(new Event('change', { bubbles: true })); }
    }
    if (event.target.id === 'eType') {
      const visible = document.getElementById('eTypeVisible');
      if (visible) visible.value = event.target.value;
      const sound = document.getElementById('eSound');
      if (sound && !document.activeElement?.matches('#eSound')) sound.value = soundForAlarmType(event.target.value);
      const repeat = document.getElementById('eRepeat');
      if (repeat && event.target.value === 'gw') repeat.value = 'gw5d';
    }
