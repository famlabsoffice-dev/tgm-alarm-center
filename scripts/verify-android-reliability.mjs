...
for (const permission of requiredAndroidPermissions) {
  if (!app.android?.permissions?.includes(permission)) {
    throw new Error(`Android permission missing: ${permission}`);
  }
}

if (!app.plugins.some((plugin) => Array.isArray(plugin) && plugin[0] === 'expo-notifications')) {
  throw new Error('expo-notifications plugin configuration missing.');
}

if (!moduleConfig.platforms.includes('android')) throw new Error('Exact-alarm module is not Android-enabled.');
if (!moduleConfig.android?.modules?.includes('expo.modules.tgmalarm.TGMExactAlarmModule')) {
  throw new Error('Exact-alarm native module is not registered for Expo autolinking.');
}
...
if (eas.build?.production?.android?.buildType !== 'app-bundle') {
  throw new Error('Production Android build is not configured as an AAB.');
}
if (eas.build?.preview?.android?.buildType !== 'apk' || eas.build?.preview?.distribution !== 'internal') {
  throw new Error('Internal Android team-test APK profile is incomplete.');
}
...
console.log('Android exact-alarm permission, battery capability hooks, notification sounds, GW cycle markers, and AAB/APK build profiles verified.');
