...
export function stripEntitlement(data: AppState): AppState { return { ...data, tier: 'free' }; }

export function makeBackup(data: AppState): Backup { return { format: FORMAT, version: VERSION, exportedAt: new Date().toISOString(), schemaVersion: 1, data: stripEntitlement(data) }; }
...
export function validateBackup(value: unknown): Backup {
  ...
  const accountIds = new Set<string>(); for (const account of data.accounts) validateAccount(account, accountIds);
  ...
  const alarmIds = new Set<string>(); for (const alarm of data.alarms) validateAlarm(alarm, accountIds, alarmIds);
  return value as unknown as Backup;
}

export function restoreBackup(payload: string | unknown): AppState {
  const parsed = typeof payload === 'string' ? (() => { if (payload.length > MAX_BACKUP_BYTES) throw new Error('Backup-Datei ist zu groß'); return JSON.parse(payload) as unknown; })() : payload;
