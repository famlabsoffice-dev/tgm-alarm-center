...
export interface Alarm {
  id: string; accountId: string; title: string; type: AlarmType; date: string; time: string; eventAtUtc: string;
...
export const TIER_LIMITS: Record<Tier, { accounts: number; alarms: number; events: number; perAccount: { bubbleAlarms: number; eventAlarms: number; individualAlarms: number; rssAlarms: number } }> = {
  ...
  godfather: { accounts: Infinity, alarms: Infinity, events: Infinity, perAccount: { bubbleAlarms: Infinity, eventAlarms: Infinity, individualAlarms: Infinity, rssAlarms: Infinity } },
};
...
function baseLocalDateTime(alarm: Alarm): Date | null {
  const utc = new Date(alarm.eventAtUtc); if (!Number.isFinite(utc.getTime())) return null; if (!validateDateTime(alarm.date, alarm.time)) return utc;
  const year = Number(alarm.date.slice(0, 4)); const month = Number(alarm.date.slice(5, 7)); const day = Number(alarm.date.slice(8, 10)); const hour = Number(alarm.time.slice(0, 2)); const minute = Number(alarm.time.slice(3, 5));
  const local = new Date(year, month - 1, day, hour, minute, 0, 0); return Number.isFinite(local.getTime()) ? local : utc;
}
...
function nextGwOccurrence(alarm: Alarm, now: Date): Date | null {
  ...
  // GW is a strict five-day cycle: the configured instant is the cycle anchor,
  // while the first schedulable occurrence is one complete cycle later.
  const cycles = Math.max(1, Math.floor(delta / FIVE_DAYS_MS) + 1);
  const candidate = new Date(base.getTime() + cycles * FIVE_DAYS_MS);
  for (let attempts = 0; attempts < 370; attempts += 1) { if (candidate.getTime() > now.getTime() && !isCompleted(alarm, candidate)) return candidate; candidate.setTime(candidate.getTime() + FIVE_DAYS_MS); }
  return null;
}
...
export function upcomingMoments(alarm: Alarm, now = new Date()): NotificationMoment[] {
  ...
  const endAt = occurrenceEnd(alarm, eventTime); if (endAt) { const endWarningAt = new Date(endAt.getTime() - 60 * 60 * 1000); if (endWarningAt.getTime() > now.getTime()) moments.push({ alarmId: alarm.id, eventTime, at: endWarningAt, kind: 'end-warning', endAt }); if (endAt.getTime() > now.getTime()) moments.push({ alarmId: alarm.id, eventTime, at: endAt, kind: 'end', endAt }); }
