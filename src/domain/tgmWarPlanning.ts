...
export interface WarPlanItem { id: string; title: string; phase: WarPlanPhase; atUtc: string; alarmId: string | null; }
export interface NextCriticalEvent { alarm: Alarm; event: Date; minutesUntil: number; kind: 'event' | 'warning' | 'end'; }
export interface WarPlan { title: string; eventType: 'gw' | 'cvc' | 'faction'; startsAt: string; battleDayAt: string | null; endsAt: string; items: WarPlanItem[]; }
...
export function nextCriticalEvent(alarms: Alarm[], now = new Date()): NextCriticalEvent | null {
  ...
  for (const alarm of alarms) {
    ...
    if (!event) continue;
    candidates.push({ alarm, event, minutesUntil: Math.max(0, Math.ceil((event.getTime() - now.getTime()) / MINUTE)), kind: 'event' });
    for (const moment of upcomingMoments(alarm, now)) {
      if (moment.at.getTime() <= now.getTime()) continue;
      candidates.push({ alarm, event: moment.at, minutesUntil: Math.max(0, Math.ceil((moment.at.getTime() - now.getTime()) / MINUTE)), kind: moment.kind === 'end' || moment.kind === 'end-warning' ? 'end' : 'warning' });
    }
  }
...
export function buildWarPlan(alarm: Alarm, now = new Date()): WarPlan | null {
  ...
  if (alarm.type === 'gwBubble') {
    ...
    return { title: alarm.title, eventType: 'gw', startsAt: startsAt.toISOString(), battleDayAt: startsAt.toISOString(), endsAt: endsAt.toISOString(), items: [
      { id: `${alarm.id}:gw-start`, title: 'GW Bubble Start', phase: 'preparation', atUtc: startsAt.toISOString(), alarmId: alarm.id },
      { id: `${alarm.id}:massacre`, title: 'Massacre Day', phase: 'battle', atUtc: startsAt.toISOString(), alarmId: alarm.id },
  ...
  if (alarm.type === 'cvc') {
    ...
    const battleDayAt = new Date(startsAt.getTime() + (cycleDays - 1) * DAY);
    const endsAt = new Date(startsAt.getTime() + cycleDays * DAY);
    return { title: alarm.title, eventType: 'cvc', startsAt: startsAt.toISOString(), battleDayAt: battleDayAt.toISOString(), endsAt: endsAt.toISOString(), items: [
  ...
  if (alarm.type === 'faction') {
    const endsAt = new Date(startsAt.getTime() + DAY);
    return { title: factionLabel(alarm.factionEvent), eventType: 'faction', startsAt: startsAt.toISOString(), battleDayAt: startsAt.toISOString(), endsAt: endsAt.toISOString(), items: [
