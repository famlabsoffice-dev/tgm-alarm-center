...
function makeOccurrence(definition: EventDefinition, start: Date, variant: string | null = null): EventOccurrence {
  ...
  return {
    id: occurrenceId(definition, start.toISOString()), definitionId: definition.id, definitionVersion: definition.version,
    startUtc: start.toISOString(), endUtc: end?.toISOString() ?? null, variant,
    status: variant ? 'communityConfirmed' : 'predicted', confidence: boundedConfidence(definition),
    sourceRefs: definition.sources.map((source) => source.id), metadata: { category: definition.category, titleKey: definition.titleKey },
  };
}
...
function localDateAt(instant: Date, timeZone: string): CalendarDate {
  ...
  return { year: requiredNumberPart(parts, 'year'), month: requiredNumberPart(parts, 'month'), day: requiredNumberPart(parts, 'day') };
}
...
function zonedLocalToUtc(date: CalendarDate, hour: number, minute: number, timeZone: string): Date {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const rendered = Date.UTC(
      requiredNumberPart(parts, 'year'), requiredNumberPart(parts, 'month') - 1,
      requiredNumberPart(parts, 'day'), requiredNumberPart(parts, 'hour'), requiredNumberPart(parts, 'minute'),
    );
...
export function generateOccurrences(definition: EventDefinition, fromUtc: Date, untilUtc: Date): EventOccurrence[] {
  ...
  switch (definition.ruleType) {
    case 'intervalFromAnchor': return anchoredOccurrences(definition, fromUtc, untilUtc);
    case 'dailyLocal': return dailyOccurrences(definition, fromUtc, untilUtc);
    case 'fixedUtc':
    ...
    case 'seasonPhase':
    case 'manual':
    case 'randomWithinWindow': return [];
    default: {
...
export function mergeOccurrenceConfirmation(occurrence: EventOccurrence, variant: string, confidence: number, sourceRef: string): EventOccurrence {
  ...
  return { ...occurrence, variant: normalizedVariant, status: 'communityConfirmed', confidence, sourceRefs: [...new Set([...occurrence.sourceRefs, sourceRef])] };
}
