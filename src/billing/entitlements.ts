...
export function normalizeVerifiedEntitlement(value: unknown, product: BillingProduct, platform: StorePlatform): VerifiedEntitlement | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const candidate = value as Record<string, unknown>;
  const productId = typeof candidate.productId === 'string' ? candidate.productId : null;
  const verifiedAt = typeof candidate.verifiedAt === 'string' && Number.isFinite(Date.parse(candidate.verifiedAt)) ? new Date(candidate.verifiedAt).toISOString() : null;
  const status = candidate.status === 'active' || candidate.status === 'expired' || candidate.status === 'pending' || candidate.status === 'revoked' ? candidate.status : null;
  if (!productId || productId !== (platform === 'ios' ? product.iosProductId : product.androidProductId) || !status || !verifiedAt) return null;
  const expiresAt = candidate.expiresAt === null ? null : typeof candidate.expiresAt === 'string' && Number.isFinite(Date.parse(candidate.expiresAt)) ? new Date(candidate.expiresAt).toISOString() : null;
