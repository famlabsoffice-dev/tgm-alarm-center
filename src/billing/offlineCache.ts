...
import { StorePlatform } from './catalog';
import { EMPTY_ENTITLEMENT, EntitlementSnapshot, isEntitlementUsable } from './entitlements';
...
function parseCached(value: unknown): CachedEntitlement | null {
  ...
  const cachedAt = typeof candidate.cachedAt === 'string' && Number.isFinite(Date.parse(candidate.cachedAt)) ? new Date(candidate.cachedAt).toISOString() : null;
  ...
  const expiresAt = item.expiresAt === null ? null : typeof item.expiresAt === 'string' && Number.isFinite(Date.parse(item.expiresAt)) ? new Date(item.expiresAt).toISOString() : null;
