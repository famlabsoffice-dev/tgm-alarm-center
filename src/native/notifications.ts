...
const TGM_NOTIFICATION_OWNER = 'tgm-alarm-center';
...
async function ensureAndroidChannels(): Promise<boolean> {
  ...
  try {
    ...
    const channels = await Promise.all([
      Notifications.getNotificationChannelAsync(SOUND_CHANNELS.pulse), Notifications.getNotificationChannelAsync(SOUND_CHANNELS.siren),
      Notifications.getNotificationChannelAsync(SOUND_CHANNELS.chime), Notifications.getNotificationChannelAsync(SOUND_CHANNELS.silent),
    ]);
...
export async function initializeNotifications(): Promise<NotificationReadiness> {
  ...
  try { const current = await Notifications.getPermissionsAsync(); permission = current.status === Notifications.PermissionStatus.GRANTED; if (!permission && current.status === Notifications.PermissionStatus.UNDETERMINED) permission = (await Notifications.requestPermissionsAsync()).status === Notifications.PermissionStatus.GRANTED; } catch { permission = false; }
  let exactAlarm = true; let recoveryPending = false;
  if (Platform.OS === 'android') { try { exactAlarm = await canScheduleExactAlarms(); const signals = await consumeRecoverySignals(); recoveryPending = signals.bootReconciliationNeeded || signals.exactAlarmPermissionChanged; } catch { exactAlarm = false; } }
...
function installResumeRecovery(): void {
  ...
  resumeSubscription = AppState.addEventListener('change', (nextState) => {
    ...
    void (async () => {
      ...
      const state = getVolatileState() ?? await loadState(); const revision = getVolatileStateRevision();
...
async function initializeNotificationsForResume(): Promise<NotificationReadiness> {
  const channel = await ensureAndroidChannels(); let permission = false;
...
export async function cancelAllScheduled(): Promise<void> {
  ...
  const revision = getVolatileStateRevision(); if (revision === lastReconciledRevision) return;
  const state = getVolatileState() ?? await loadState(); await reconcileScheduledNotifications(state.alarms, state.notificationPreferences, revision);
}
...
function eventLabelFor(alarm: Alarm): string {
  if (alarm.type === 'gwBubble') return 'Massacre Day / GW';
  if (alarm.type === 'bubble') return 'Bubble';
  if (alarm.type === 'cvc') return 'CvC · City vs City';
  if (alarm.type === 'faction') return alarm.factionEvent === 'oakvale' ? 'Oakvale' : alarm.factionEvent === 'undergroundMarket' ? 'Underground Market' : 'Faction Battle Event';
  if (alarm.type === 'custom') return 'TGM Event';
  if (alarm.type === 'individual') return 'Individual Timer';
  return 'RSS Timer';
}
function contentFor(alarm: Alarm, moment: NotificationMoment, preferences: NotificationPreferences, accountName: string, ownershipKey: string): Notifications.NotificationContentInput {
  const eventLabel = eventLabelFor(alarm);
  const isWarning = moment.kind === 'warning'; const isBattleDay = moment.kind === 'battle-day'; const isEndWarning = moment.kind === 'end-warning'; const isEnd = moment.kind === 'end';
  const accountLabel = accountName.trim() || 'Unbekannter Account';
  const title = isBattleDay ? `TGM · ${accountLabel} · ${eventLabel} · BATTLE DAY` : isEnd ? `TGM · ${accountLabel} · ${alarm.title} endet` : isEndWarning ? `TGM · ${accountLabel} · ${alarm.title}` : isWarning ? `TGM · ${accountLabel} · ${alarm.title}` : `TGM ALARM CENTER · ${accountLabel} · ${alarm.title}`;
  const body = isBattleDay ? `${eventLabel}: Battle Day ist jetzt aktiv.` : isEndWarning ? `${accountLabel}: ${alarm.title}: Das Event endet um ${moment.endAt?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.` : isEnd ? `${accountLabel}: ${alarm.title} endet jetzt.` : isWarning ? `${accountLabel}: ${eventLabel} beginnt um ${moment.eventTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.` : `${accountLabel}: ${eventLabel} beginnt jetzt.`;
  const soundEnabled = isWarning || isEndWarning ? preferences.warningSound : preferences.eventSound;
  return { title, body, sound: soundEnabled ? soundFor(alarm.sound) : undefined, vibrate: preferences.vibration ? [0, 250, 150, 250] : undefined, data: { tgmOwner: TGM_NOTIFICATION_OWNER, accountId: alarm.accountId, alarmId: alarm.id, eventTime: moment.eventTime.toISOString(), kind: moment.kind, warningMinutes: moment.warningMinutes ?? null, endAt: moment.endAt?.toISOString() ?? null, ownershipKey }, categoryIdentifier: isWarning || isEndWarning ? 'tgm-warning' : 'tgm-event', interruptionLevel: preferences.criticalAlerts ? 'timeSensitive' : 'active', ...(Platform.OS === 'android' ? { channelId: channelFor(alarm, soundEnabled) } : {}) };
}
async function readRegistry(): Promise<NotificationRegistry> {
  try { const AsyncStorage = (await import('@react-native-async-storage/async-storage')).default; const raw = await AsyncStorage.getItem(NOTIFICATION_REGISTRY_KEY); if (!raw) return {}; const value: unknown = JSON.parse(raw); if (!value || typeof value !== 'object' || Array.isArray(value)) return {}; return Object.fromEntries(Object.entries(value as Record<string, unknown>).filter((entry): entry is [string, string] => typeof entry[0] === 'string' && typeof entry[1] === 'string')); } catch { return {}; }
}
async function writeRegistry(registry: NotificationRegistry): Promise<void> { const AsyncStorage = (await import('@react-native-async-storage/async-storage')).default; await AsyncStorage.setItem(NOTIFICATION_REGISTRY_KEY, JSON.stringify(registry)); }
function pendingOwnershipKey(notification: Notifications.NotificationRequest): string | null {
  const data = notification.content.data as { tgmOwner?: unknown; ownershipKey?: unknown; alarmId?: unknown; eventTime?: unknown; kind?: unknown; warningMinutes?: unknown } | undefined;
  if (data?.tgmOwner !== TGM_NOTIFICATION_OWNER) return null;
...
async function schedulePlanEntry(entry: NotificationPlanEntry, alarm: Alarm, preferences: NotificationPreferences, accountName: string, now: Date): Promise<string> {
  const moment = upcomingMoments(alarm, now).find((candidate) => candidate.kind === entry.kind && candidate.eventTime.toISOString() === entry.eventTime && (candidate.warningMinutes ?? null) === (entry.warningMinutes ?? null));
...
async function reconcileScheduledNotificationsInternal(alarms: readonly Alarm[], preferences: NotificationPreferences, revision?: number): Promise<void> {
  ...
  const effectiveRevision = revision ?? getVolatileStateRevision(); if (effectiveRevision === lastReconciledRevision) return;
  const now = new Date(); const plan = buildNotificationPlan([...alarms], preferences, now); const accountState = getVolatileState() ?? await loadState();
  const accountNames = new Map(accountState.accounts.map((account) => [account.id, account.name])); const alarmById = new Map(alarms.map((alarm) => [alarm.id, alarm]));
  const desired = new Map(plan.map((entry) => [notificationOwnershipKey(entry), entry])); const pending = await Notifications.getAllScheduledNotificationsAsync(); const registry = await readRegistry(); const current = new Map<string, string>();
  for (const notification of pending) {
    const key = pendingOwnershipKey(notification); if (!key) continue;
  ...
  for (const [key, entry] of desired) {
    ...
    const alarm = alarmById.get(entry.alarmId); if (!alarm) continue;
    const id = await schedulePlanEntry(entry, alarm, preferences, accountNames.get(alarm.accountId) ?? alarm.accountId, now); current.set(key, id);
  }
  ...
  await writeRegistry(Object.fromEntries(current)); if (effectiveRevision === getVolatileStateRevision()) lastReconciledRevision = effectiveRevision;
}
export async function reconcileScheduledNotifications(alarms: readonly Alarm[], preferences: NotificationPreferences, revision?: number): Promise<void> {
  ...
  reconciliationQueue = run.catch(() => undefined); await run;
}
export async function scheduleAlarm(alarm: Alarm, preferences: NotificationPreferences, _accountName?: string): Promise<string[]> {
  if (Platform.OS === 'web') return [];
  const revision = getVolatileStateRevision(); if (revision === lastReconciledRevision) return [];
  const state = getVolatileState() ?? await loadState(); await reconcileScheduledNotifications(state.alarms, preferences, revision); const registry = await readRegistry();
