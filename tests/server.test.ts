...
test('accepts a valid Google Pub/Sub OIDC token', () => {
  ...
  const signed = token(privateKey.export({ type: 'pkcs8', format: 'pem' }).toString(), { iss: 'https://accounts.google.com', aud: 'https://billing.example.test/google-rtdn', iat: now - 10, exp: now + 300 });
...
test('rejects a Google Pub/Sub token with a wrong audience', () => {
  const { privateKey, publicKey } = generateKeyPairSync('rsa', { modulusLength: 2048 });
  const now = Math.floor(Date.now() / 1000);
  const signed = token(privateKey.export({ type: 'pkcs8', format: 'pem' }).toString(), { iss: 'https://accounts.google.com', aud: 'https://attacker.example.test', iat: now - 10, exp: now + 300 });
