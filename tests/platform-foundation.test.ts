...
test('normalizes a canonical external event deterministically', () => {
  const event = normalizeExternalEvent({
    source: 'partner',
    sourceId: 'tgmhub',
    sourceEventId: 'lucky-sprint-2026-09-06',
    title: 'Lucky Sprint',
    startAtUtc: '2026-09-06T18:00:00.000Z',
    endAtUtc: '2026-09-07T18:00:00.000Z',
    timezone: 'Europe/Berlin',
    priority: 'high',
    alarmType: 'custom',
    warningMinutes: [60, 15, 15],
    sound: 'chime',
...
test('rejects invalid UTC and reversed end timestamps', () => {
  const errors = validateExternalEvent({
    source: 'partner', sourceId: 'tgmhub', sourceEventId: 'bad', title: 'Bad',
    startAtUtc: '2026-09-06T18:00:00.000Z', endAtUtc: '2026-09-06T17:00:00.000Z',
  });
...
test('registry prevents duplicate adapters and preserves source identity', () => {
  ...
  assert.throws(() => registry.normalize('tgmhub', {
    source: 'partner', sourceId: 'other', sourceEventId: '1', title: 'Mismatch', startAtUtc: '2026-09-06T18:00:00.000Z',
  }), /identity mismatch/);
});

test('converts a normalized event into an existing Alarm without changing alarm-domain semantics', () => {
  const event = normalizeExternalEvent({
    source: 'partner', sourceId: 'tgmhub', sourceEventId: 'gw-1', title: 'GW Event',
    startAtUtc: '2026-09-06T18:00:00.000Z', priority: 'critical', alarmType: 'gwBubble',
    warningMinutes: [60, 30, 15], sound: 'siren', repeat: 'once',
  }, new Date('2026-09-06T05:00:00.000Z'));
...
test('deduplicates by canonical id and retains the newest event', () => {
  const base = {
    source: 'partner' as const, sourceId: 'tgmhub', sourceEventId: 'same', title: 'Event', startAtUtc: '2026-09-06T18:00:00.000Z',
  };
