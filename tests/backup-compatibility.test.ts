...
const baseState = (tier: AppState['tier'] = 'free', activeAccountId: string | null = 'account-1', testConfirmedAt: string | null = null): AppState => ({
  ...
  alarms: [{
    ...
    date: '2026-09-05',
    time: '12:00',
    eventAtUtc: '2026-09-05T10:00:00.000Z',
    warnings: [60, 15],
    repeat: 'once',
    sound: 'pulse',
    active: true,
...
test('schema-1 validator accepts every supported state representation', () => {
  const variants = [
    baseState('free', null, null),
    baseState('streetBoss', 'account-1', '2026-09-05T01:00:00.000Z'),
    baseState('caporegime'),
    baseState('underboss'),
    baseState('boss'),
    baseState('godfather'),
  ];
...
test('restore strips a forged paid tier even when the rest of the payload is valid', () => {
  const forged: Backup = { ...makeBackup(baseState('godfather')), data: { ...makeBackup(baseState('godfather')).data, tier: 'godfather' } };
...
test('validator rejects malformed and oversized restore payloads before state use', () => {
  const backup = makeBackup(baseState());
