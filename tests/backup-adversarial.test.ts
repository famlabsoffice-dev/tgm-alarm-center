...
test('backup export strips paid entitlement', () => assert.equal(makeBackup(state).data.tier, 'free'));

test('backup restore rejects an alarm referencing a missing account', () => {
  const backup = makeBackup(state);
  const malformed = { ...backup, data: { ...backup.data, alarms: [{ id: 'bad', accountId: 'missing', title: 'Bad', type: 'custom', date: '2026-09-05', time: '10:00', eventAtUtc: '2026-09-05T08:00:00.000Z', warnings: [15], repeat: 'once', sound: 'pulse', active: true, protected: false, completedOccurrences: {}, createdAt: '2026-09-05T00:00:00.000Z', updatedAt: '2026-09-05T00:00:00.000Z' }] } };
...
test('backup restore rejects duplicate account and alarm identities', () => {
  const backup = makeBackup(state);
  const duplicateAccount = { ...backup, data: { ...backup.data, accounts: [backup.data.accounts[0], backup.data.accounts[0]] } };
...
test('backup restore rejects oversized payload before parsing', () => assert.throws(() => restoreBackup('x'.repeat(512 * 1024 + 1)), /zu groß/));
...
test('backup restore rejects malformed JSON and invalid active-account references', () => {
  ...
  const base = makeBackup(state);
  const malformed = { ...base, data: { ...base.data, activeAccountId: 'missing' } };
