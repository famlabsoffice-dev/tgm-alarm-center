...
const baseEvent = (overrides: Partial<AlarmableEvent> = {}): AlarmableEvent => ({
  id: 'manual:device:1', source: 'manual', sourceId: 'device', sourceEventId: '1', title: 'Super Weapon', description: 'Start', category: 'event',
  startAtUtc: '2026-09-07T12:00:00.000Z', endAtUtc: null, timezone: 'Europe/Berlin', priority: 'critical', alarmType: 'custom', repeat: 'once',
  warningMinutes: [60, 15], sound: 'siren', action: 'notify', metadata: {}, createdAt: '2026-09-06T00:00:00.000Z', updatedAt: '2026-09-06T00:00:00.000Z', ...overrides,
});

const baseAlarm = (overrides: Partial<Alarm> = {}): Alarm => ({
  id: 'alarm-1', accountId: 'account-1', title: 'Existing', type: 'custom', date: '2026-09-08', time: '14:00', eventAtUtc: '2026-09-08T12:00:00.000Z',
  warnings: [15], repeat: 'once', sound: 'chime', active: true, protected: false, completedOccurrences: {}, createdAt: '2026-09-06T00:00:00.000Z', updatedAt: '2026-09-06T00:00:00.000Z', ...overrides,
});
...
test('upsertInboxEvent adds and updates one canonical event without duplication', () => {
  ...
  assert.equal(updated[0]?.event.title, 'Super Weapon updated');
});
...
test('recommendEvents prioritizes critical upcoming events', () => {
  ...
  assert.equal(recommendations[0]!.event.id, critical.id);
...
test('dismiss and add states are explicit', () => {
  ...
  assert.equal(added[0]?.addedAlarmId, 'event:manual:device:1');
});

test('canAddEventAlarm enforces the existing event category limit', () => {
  const decision = canAddEventAlarm('free', [baseAlarm()], 'account-1', baseEvent());
  assert.equal(decision.allowed, false);
  assert.match(decision.reason ?? '', /Kategorie/);
});
...
test('alarmFromEvent keeps the same instant while presenting local device date and time', () => {
  ...
  assert.equal(alarm.eventAtUtc, '2026-09-07T12:00:00.000Z');
  assert.equal(alarm.id, 'event:manual:device:1');
  assert.equal(localDateTimeToUtc(alarm.date, alarm.time), event.startAtUtc);
});
