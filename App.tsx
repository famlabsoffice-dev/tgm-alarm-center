...
export default function App() {
  ...
  const [nativeView, setNativeView] = useState<'alarms' | 'dashboard' | 'cycle' | 'backup' | 'accounts' | 'sounds' | 'plans' | 'settings'>('alarms');
  ...
  /> : nativeView === 'cycle' ? <CycleScreen alarms={state.alarms} onBack={() => setNativeView('alarms')} onOpenEdit={openEdit} /> : nativeView === 'backup' ? <BackupScreen onBack={() => setNativeView('alarms')} onExport={exportCurrentBackup} onImport={importBackup} onReset={resetLocalData} updatedAt={nowIso()} alarmCount={state.alarms.length} /> : nativeView !== 'dashboard' ? <UtilityScreen view={nativeView} state={state} activeAccount={activeAccount} onBack={() => setNativeView('alarms')} onNavigate={(nextView) => setNativeView(nextView)} onCreate={quickCreate} onExport={exportCurrentBackup} onImport={importBackup} onReset={resetLocalData} onPreference={updatePreference} onTestNotification={runDeviceTest} onTierConfirmed={confirmStoreTier} /> : (
...
type UtilityView = 'accounts' | 'sounds' | 'plans' | 'settings';
type PreferenceKey = 'warningSound' | 'eventSound' | 'vibration' | 'criticalAlerts';
type UtilityScreenProps = { view: UtilityView; state: AppState; activeAccount: AppState['accounts'][number] | null; onBack: () => void; onNavigate: (view: 'alarms' | 'cycle' | 'backup' | 'accounts' | 'sounds' | 'plans' | 'settings' | 'dashboard') => void; onCreate: (key: TemplateKey) => void; onExport: () => void; onImport: () => Promise<void>; onReset: () => void; onPreference: (key: PreferenceKey, value: boolean) => void; onTestNotification: () => Promise<void>; onTierConfirmed: (tier: Tier) => void };

function UtilityScreen({ view, state, activeAccount, onBack, onNavigate, onCreate, onExport, onImport, onReset, onPreference, onTestNotification, onTierConfirmed }: UtilityScreenProps): React.ReactElement {
  const titles: Record<UtilityView, string> = { accounts: 'ACCOUNTS', sounds: 'GAMING-TÖNE', plans: 'PLÄNE & PREISE', settings: 'EINSTELLUNGEN' };
  const pref = state.notificationPreferences;
  return <SafeAreaView style={modernStyles.screen}><StatusBar hidden /><View style={modernStyles.screenHeader}><Pressable accessibilityRole="button" onPress={onBack} style={modernStyles.backButton}><Text style={modernStyles.backText}>‹</Text></Pressable><Text style={modernStyles.headerTitle}>{titles[view]}</Text><View style={modernStyles.headerSpacer} /></View><ScrollView contentContainerStyle={modernStyles.content} showsVerticalScrollIndicator={false}>
    {view === 'accounts' ? <><View style={modernStyles.utilityHero}><Text style={modernStyles.eyebrowModern}>PROFILE</Text><Text style={modernStyles.utilityTitle}>Deine Accounts</Text><Text style={modernStyles.cycleMuted}>{state.accounts.length} Account{state.accounts.length === 1 ? '' : 's'} · {state.alarms.length} Alarme lokal gespeichert · Aktiv: {activeAccount?.name ?? 'kein Account'}</Text></View>{state.accounts.length ? state.accounts.map((account) => <View key={account.id} style={[modernStyles.utilityRow, account.id === state.activeAccountId && modernStyles.utilityRowActive]}><View style={[modernStyles.accountColorModern, { backgroundColor: account.color }]} /><View style={modernStyles.utilityCopy}><Text style={modernStyles.backupRowTitle}>{account.name}</Text><Text style={modernStyles.backupRowText}>{state.alarms.filter((alarm) => alarm.accountId === account.id).length} Alarme · {account.id === state.activeAccountId ? 'Aktiv' : 'Verfügbar'}</Text></View></View>) : <View style={modernStyles.emptyCard}><Text style={modernStyles.emptyTitle}>Noch kein Account</Text><Text style={modernStyles.emptyText}>Beim ersten Alarm wird automatisch ein lokaler Account angelegt.</Text><Pressable onPress={() => onCreate('bubble')} style={modernStyles.primaryButton}><Text style={modernStyles.primaryButtonText}>ERSTEN ALARM ANLEGEN</Text></Pressable></View>}</> : null}
    {view === 'sounds' ? <><View style={modernStyles.utilityHero}><Text style={modernStyles.eyebrowModern}>AUDIO ENGINE</Text><Text style={modernStyles.utilityTitle}>Gaming-Alarmtöne</Text><Text style={modernStyles.cycleMuted}>Lokale Töne für Vorwarnungen, Hauptereignisse und Zyklus-Enden.</Text></View><View style={modernStyles.utilityRow}><Text style={modernStyles.backupIcon}>♪</Text><View style={modernStyles.utilityCopy}><Text style={modernStyles.backupRowTitle}>Audio-Bereitschaft</Text><Text style={modernStyles.backupRowText}>{state.notificationPreferences.preview ? 'Vorschau aktiviert' : 'Lokale Vorschau auf dem Gerät'}</Text></View><Text style={modernStyles.synced}>{state.notificationPreferences.preview ? 'BEREIT' : 'LOKAL'}</Text></View><Pressable accessibilityRole="button" onPress={onTestNotification} style={modernStyles.primaryButton}><Text style={modernStyles.primaryButtonText}>GERÄTETEST STARTEN</Text></Pressable></> : null}
    {view === 'plans' ? <BillingPanel currentTier={state.tier} onTierConfirmed={onTierConfirmed} /> : null}
    {view === 'settings' ? <><View style={modernStyles.utilityHero}><Text style={modernStyles.eyebrowModern}>KONFIGURATION</Text><Text style={modernStyles.utilityTitle}>Alarmverhalten</Text><Text style={modernStyles.cycleMuted}>Benachrichtigungen und lokale Datenkontrolle.</Text></View>{([['warningSound', 'Vorwarnungen mit Ton'], ['eventSound', 'Hauptereignisse mit Ton'], ['vibration', 'Vibration auf Mobilgeräten'], ['criticalAlerts', 'Zeitkritische Alarmstärke']] as [PreferenceKey, string][]).map(([key, label]) => <View key={key} style={modernStyles.settingModern}><Text style={modernStyles.backupRowTitle}>{label}</Text><Switch value={pref[key]} onValueChange={(value) => onPreference(key, value)} trackColor={{ false: '#283038', true: '#296D3B' }} thumbColor={pref[key] ? MODERN_COLORS.green : '#B7BEC5'} /></View>)}<Text style={modernStyles.sectionModern}>DATEN AUF DIESEM GERÄT</Text><Pressable onPress={onExport} style={modernStyles.backupRow}><Text style={modernStyles.backupIcon}>⇧</Text><View style={modernStyles.backupCopy}><Text style={modernStyles.backupRowTitle}>Backup exportieren</Text><Text style={modernStyles.backupRowText}>Accounts, Alarme und Einstellungen sichern</Text></View><Text style={modernStyles.cardChevron}>›</Text></Pressable><Pressable onPress={onImport} style={modernStyles.backupRow}><Text style={modernStyles.backupIcon}>⇩</Text><View style={modernStyles.backupCopy}><Text style={modernStyles.backupRowTitle}>Backup importieren</Text><Text style={modernStyles.backupRowText}>Lokale Daten wiederherstellen</Text></View><Text style={modernStyles.cardChevron}>›</Text></Pressable><Pressable onPress={onReset} style={modernStyles.backupDanger}><Text style={modernStyles.backupDangerTitle}>Alle lokalen Daten löschen</Text><Text style={modernStyles.dangerChevron}>›</Text></Pressable></> : null}
  </ScrollView><View style={modernStyles.bottomNav}><Pressable onPress={onBack} style={modernStyles.navItem}><Text style={modernStyles.navIcon}>⌂</Text><Text style={modernStyles.navLabel}>DASHBOARD</Text></Pressable><Pressable onPress={() => onNavigate('alarms')} style={modernStyles.navItem}><Text style={modernStyles.navIcon}>♧</Text><Text style={modernStyles.navLabel}>ALARME</Text></Pressable><Pressable onPress={() => onNavigate('cycle')} style={modernStyles.navItem}><Text style={modernStyles.navIcon}>◌</Text><Text style={modernStyles.navLabel}>ZYKLEN</Text></Pressable><Pressable style={modernStyles.navItem}><Text style={modernStyles.navIconSelected}>⚙</Text><Text style={modernStyles.navLabelSelected}>SYSTEM</Text></Pressable></View></SafeAreaView>;
}
