...
import { Tier } from '../src/domain/alarm';
...
export class BillingRepository {
  ...
  async getActiveEntitlement(userId: string, at = Date.now()): Promise<StoredEntitlement | null> {
    ...
    return entitlements.filter((entitlement) => entitlement.status === 'active' && (entitlement.expiresAt === null || Date.parse(entitlement.expiresAt) > at)).sort((left, right) => tierRank(right.tier) - tierRank(left.tier))[0] ?? null;
  }
