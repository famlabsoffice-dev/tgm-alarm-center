...
function rawEcdsaSignatureToDer(signature: Buffer): Buffer {
  ...
  const sequence = Buffer.concat([integer(0), integer(32)]);
...
function verifyCertificateChain(certificates: string[], trustedRootPem: string): X509Certificate {
  ...
  for (let index = 0; index < chain.length - 1; index += 1) {
    ...
    if (!issuer || !subject || !issuer.checkIssued(subject) || !issuer.verify(subject.publicKey)) throw new SecurityError('Apple-Zertifikatskette ist ungültig.');
  }
...
export function verifyAppleJws(compactJws: string, trustedRootPem: string): VerifiedJws {
  ...
  if (parts.length !== 3 || parts.some((part) => !part)) throw new SecurityError('Apple-JWS muss drei Segmente enthalten.');
  const [headerPart, payloadPart, signaturePart] = parts;
  const header = decodeJson(headerPart as string);
  const payload = decodeJson(payloadPart as string);
  if (header.alg !== 'ES256' || header.typ !== 'JWT') throw new SecurityError('Apple-JWS-Algorithmus oder Typ ist ungültig.');
  ...
  const signature = rawEcdsaSignatureToDer(decodeBase64Url(signaturePart as string));
  if (!verifySignature('sha256', Buffer.from(`${headerPart}.${payloadPart}`, 'ascii'), leaf.publicKey, signature)) throw new SecurityError('Apple-JWS-Signatur ist ungültig.');
...
export function verifyGoogleOidcClaims(token: string, trustedPublicKeyPem: string, expectedAudience: string, now = Math.floor(Date.now() / 1000)): VerifiedGoogleOidc {
  ...
  if (parts.length !== 3 || parts.some((part) => !part)) throw new SecurityError('Google-OIDC-Token muss drei Segmente enthalten.');
  const [headerPart, claimsPart, signaturePart] = parts;
  const header = decodeJson(headerPart as string);
  const claims = decodeJson(claimsPart as string);
  ...
  const signature = decodeBase64Url(signaturePart as string);
