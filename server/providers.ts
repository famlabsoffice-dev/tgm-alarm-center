...
import { BillingProduct, getBillingCatalog, storeProductId, StorePlatform } from '../src/billing/catalog';
...
function findProduct(platform: StorePlatform, productId: string): BillingProduct {
  const product = getBillingCatalog().products.find((candidate) => storeProductId(candidate, platform) === productId);
  if (!product) throw new SecurityError('Store-Produkt ist nicht im serverseitigen Katalog freigegeben.');
  return product;
}

function stringClaim(value: unknown, name: string): string {
  if (typeof value !== 'string' || value.length === 0) throw new SecurityError(`Fehlender Claim: ${name}`);
  return value;
}
...
export class ApplePurchaseVerifier {
  ...
  verifySignedTransaction(signedTransactionInfo: string, userId: string): VerifiedPurchase {
    ...
    const product = findProduct('ios', productId);
    const storeUserId = typeof transaction.appAccountToken === 'string' && transaction.appAccountToken.length > 0 ? transaction.appAccountToken : stringClaim(transaction.originalTransactionId, 'originalTransactionId');
    ...
    const now = Date.now();
    const status: VerifiedPurchase['status'] = transaction.revocationDate !== undefined ? 'revoked' : expiresAt && Date.parse(expiresAt) <= now ? 'expired' : 'active';
    return { userId, platform: 'ios', product, transactionId, status, environment, expiresAt };
  }
}

interface GoogleAccessTokenResponse { access_token?: string; expires_in?: number; }
...
export class GooglePurchaseVerifier {
  ...
  async verifySubscription(productId: string, purchaseToken: string, userId: string): Promise<VerifiedPurchase> {
    const product = findProduct('android', productId);
    ...
    const storeUserId = accountIdentifiers && typeof accountIdentifiers === 'object' && !Array.isArray(accountIdentifiers) ? (accountIdentifiers as Record<string, unknown>).obfuscatedExternalAccountId : null;
    assertUserBinding(userId, stringClaim(storeUserId, 'obfuscatedExternalAccountId'));
    const state = payload.subscriptionState;
    const status: VerifiedPurchase['status'] = state === 'SUBSCRIPTION_STATE_ACTIVE' || state === 'SUBSCRIPTION_STATE_IN_GRACE_PERIOD' ? 'active' : state === 'SUBSCRIPTION_STATE_CANCELED' || state === 'SUBSCRIPTION_STATE_EXPIRED' ? 'expired' : 'pending';
  ...
  async verifyOneTime(productId: string, purchaseToken: string, userId: string): Promise<VerifiedPurchase> {
    ...
    const storeUserId = payload.obfuscatedExternalAccountId;
    assertUserBinding(userId, stringClaim(storeUserId, 'obfuscatedExternalAccountId'));
